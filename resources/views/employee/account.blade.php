    ...
    <div class="row">
        <div class="col-md-4 border-right">
            <div class="d-flex flex-column align-items-center text-center p-3 py-5">
                ...
                <span class="font-weight-bold">{{$user->employee->fullname}}</span>
                <span class="text-black-50">{{$user->employee->phone}}</span>
                <span>{{$user->employee->address}}</span>
            </div>
        </div>
        <div class="col-md-8">
            <div class="p-3 py-5">
                <form method="POST" action="{{route('updateEmployeeAccount')}}" class="mt-2">
                    @csrf
                    @method('patch')
                    ...
                    <div class="form-group mt-2">
                        ...
                        <input type="text" id="fullname" name="fullname" class="form-control" value="{{$user->employee->fullname}}" >
                    </div>
                    <div class="form-group mt-2">
                        <label class="mb-1" for="address">Địa chỉ</label>
                        <input type="text" id="address" name="address" class="form-control" value="{{$user->employee->address}}" >
                    </div>
                    <div class="form-group mt-2">
                        <label class="mb-1" for="phone">Số điện thoai</label>
                        <input type="text" id="phone" name="phone" class="form-control" value="{{$user->employee->phone}}" >
                    </div>
                    <div class="form-group mt-2">
                        <label class="mb-1" for="birthday">Ngày sinh</label>
                        <input type="date" id="birthday" name="birthday" class="form-control" value="{{$user->employee->birthday}}" >
                    </div>
                    <div class="form-group mt-2">
                        <label class="mb-1" for="gender">Giới tính</label>
                        <input type="text" name="gender" id="gender" class="form-control" value="{{$user->employee->gender}}" disabled>
                    </div>
